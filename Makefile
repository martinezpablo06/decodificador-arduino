...
PORT = /dev/ttyACM0
TARGET = printArrayWithAssembler
ARDUINO = /usr/share/arduino/hardware/arduino/cores/arduino
LIB=/usr/share/arduino/hardware/arduino/variants/standard
SRC = $(ARDUINO)/wiring_shift.c $(ARDUINO)/wiring_analog.c \
  $(ARDUINO)/wiring_pulse.c $(ARDUINO)/wiring_digital.c \
  $(ARDUINO)/WInterrupts.c  $(ARDUINO)/wiring.c 
CXXSRC = $(TARGET).cpp $(ARDUINO)/CDC.cpp $(ARDUINO)/IPAddress.cpp \
 $(ARDUINO)/main.cpp $(ARDUINO)/Tone.cpp $(ARDUINO)/Print.cpp $(ARDUINO)/WMath.cpp \
 $(ARDUINO)/Stream.cpp  $(ARDUINO)/USBCore.cpp  $(ARDUINO)/HardwareSerial.cpp \
 $(ARDUINO)/WString.cpp 
ASRC = keyDecode.S arrayDecode.S holaSearch.S manejoPuertosEntrada.S manejoPuertosSalida.S
...
CCEXTRA= -fno-exceptions -ffunction-sections -fdata-sections
#CEXTRA = -Wa,-adhlns=$(<:.c=.lst)

CFLAGS = $(CDEBUG) $(CDEFS) $(CINCS) -O$(OPT) $(CWARN) $(CSTANDARD) $(CEXTRA)
CXXFLAGS = $(CDEFS) $(CCEXTRA) $(CINCS) -O$(OPT)
...
AVRDUDE_FLAGS = -v -v -v -v -p $(MCU) -P $(AVRDUDE_PORT) -c $(AVRDUDE_PROGRAMMER) \
  -b $(UPLOAD_RATE) -D
#AVRDUDE_FLAGS = -p $(MCU) -P $(AVRDUDE_PORT)-carduino
...
OBJ = $(ASRC:.S=.o) $(SRC:.c=.o) $(CXXSRC:.cpp=.o)
...
$(TARGET).elf: $(OBJ)
	$(CC) -Os -Wl,--gc-sections -mmcu=atmega328p  $(OBJ)  --output $@ $(LDFLAGS)
